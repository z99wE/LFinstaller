...
class MMPluginMixin:
    ...
    def _get_video_token_metadata(
        self,
        videos: list["VideoInput"],
        processor: "MMProcessor") -> Optional[dict[str, Any]]:
    ...
    def _get_mm_inputs(
        ...
        imglens: list[int] | None = None) -> dict[str, "torch.Tensor"]:
        ...
        if len(videos) != 0:
            ...
            videos = self._regularize_videos(
                ...
                video_maxlen=getattr(processor, "video_maxlen", 128))["videos"]
        ...
        if len(audios) != 0:
            ...
            audios = self._regularize_audios(
                audios,
                sampling_rate=getattr(processor, "audio_sampling_rate", 16000))["audios"]
            mm_inputs.update(
                feature_extractor(
                    audios,
...
class BasePlugin(MMPluginMixin):
    def process_messages(
        ...
        videos: list["VideoInput"],
        audios: list["AudioInput"],
        processor: Optional["MMProcessor"]) -> list[dict[str, str]]:
    ...
    def process_token_ids(
        ...
        processor: Optional["MMProcessor"]) -> tuple[list[int], list[int] | None]:
    ...
    def get_mm_inputs(
        ...
        processor: Optional["MMProcessor"]) -> dict[str, Union[list[int], "torch.Tensor"]]:
...
class Gemma4Plugin(BasePlugin):
    ...
    def _get_mm_inputs(
        ...
        videos: list["VideoInput"],
        audios: list["AudioInput"],
        processor: "MMProcessor") -> dict[str, Union[list[int], "torch.Tensor"]]:
...
class InternVLPlugin(BasePlugin):
    ...
    def process_messages(
        ...
        videos: list["VideoInput"],
        audios: list["AudioInput"],
        processor: Optional["ProcessorMixin"]) -> list[dict[str, str]]:
        self._validate_input(processor, images, videos, audios)
        self._validate_messages(messages, images, videos, audios)
    ...
    def get_mm_inputs(
        ...
        audlens: list[int],
        batch_ids: list[list[int]],
        processor: Optional["ProcessorMixin"]) -> dict[str, Union[list[int], "torch.Tensor"]]:
        self._validate_input(processor, images, videos, audios)
        mm_inputs = self._get_mm_inputs(images, videos, audios, processor)
...
class Qwen2VLPlugin(BasePlugin):
    ...
    def _get_qwen_video_stream_metadata(
        ...
        video: "VideoInput",
        video_fps: float,
        video_maxlen: int) -> Optional[dict[str, Any]]:
    ...
    def _get_mm_inputs(
        ...
        videos: list["VideoInput"],
        audios: list["AudioInput"],
        processor: "MMProcessor") -> dict[str, "torch.Tensor"]:
        image_processor: BaseImageProcessor = getattr(processor, "image_processor", None)
